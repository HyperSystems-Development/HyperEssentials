...
import com.hyperessentials.config.modules.DebugConfig;
...
public final class SentryIntegration {
  ...
  private static final String DSN =
      "https://bad0e83213ebe686adbfc48d79d4cefc@o4510966614589440.ingest.us.sentry.io/4511074700754944";
   ...
   * Does nothing if Sentry is disabled.
  ...
  public static void init(@NotNull DebugConfig config) {
    ...
    try {
      Sentry.init(options -> {
        options.setDsn(DSN);
        options.setRelease("hyperessentials@" + BuildInfo.VERSION);
        ...
        // Highlight HyperEssentials frames in stack traces
        options.addInAppInclude("com.hyperessentials");
  ...
  public static boolean sendTestEvent() {
    ...
    try {
      Exception testException = new RuntimeException("HyperEssentials Sentry test event — this is not a real error");
