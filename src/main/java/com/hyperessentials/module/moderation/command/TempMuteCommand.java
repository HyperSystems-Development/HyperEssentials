 ...
 */
public class TempMuteCommand extends AbstractPlayerCommand {

  private final ModerationModule module;

  public TempMuteCommand(@NotNull ModerationModule module) {
  ...
  protected void execute(@NotNull CommandContext ctx,
    ...
    if (parts.length < 3) {
      ctx.sendMessage(CommandUtil.error("Usage: /tempmute <player> <duration> [reason...]"));
      return;
    }

    String targetName = parts[1];
    long durationMs = DurationParser.parse(parts[2]);
    if (durationMs <= 0) {
      ctx.sendMessage(CommandUtil.error("Invalid duration. Examples: 1h, 7d, 30m, 1d12h"));
      return;
    }

    String reason = parts.length > 3 ? joinArgs(parts, 3) : null;
