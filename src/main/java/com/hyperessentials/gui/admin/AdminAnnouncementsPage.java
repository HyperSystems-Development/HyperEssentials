...
public class AdminAnnouncementsPage extends InteractiveCustomUIPage<AdminPageData> {
  ...
  private void buildListView(@NotNull UICommandBuilder cmd, @NotNull UIEventBuilder events) {
    ...
    // Action button row: Create + Events (styled via template)
    cmd.append("#IndexCards", UIPaths.ADMIN_ANNOUNCEMENT_HEADER);
    ...
    if (all.isEmpty()) {
      ...
      List<Announcement> pageItems = sorted.subList(startIdx, endIdx);

      int i = 1; // index 0 is the action button row
      for (Announcement ann : pageItems) {
    ...
    // Hide pagination in empty state
    if (all.isEmpty()) {
  ...
  private void buildEditView(@NotNull UICommandBuilder cmd, @NotNull UIEventBuilder events) {
    ...
    List<DropdownEntryInfo> typeEntries = new ArrayList<>();
    for (AnnouncementType t : AnnouncementType.values()) {
      typeEntries.add(new DropdownEntryInfo(LocalizableString.fromString(t.name()), t.name()));
    }
    cmd.set(edit + " #EditTypeDropdown.Entries", typeEntries);
    cmd.set(edit + " #EditTypeDropdown.Value", ann.type().name());
    ...
    // Wire type dropdown change
    events.addEventBinding(
        CustomUIEventBindingType.ValueChanged, edit + " #EditTypeDropdown",
        EventData.of("Button", "TypeChanged")
            .append("@FilterValue", edit + " #EditTypeDropdown.Value"), false
    );
  ...
  private void buildEventsView(@NotNull UICommandBuilder cmd, @NotNull UIEventBuilder events) {
    ...
    // Back button + title (template)
    cmd.append("#IndexCards", UIPaths.ADMIN_ANNOUNCEMENT_EVENTS_HEADER);
    events.addEventBinding(
    ...
    for (int t = 0; t < toggles.length; t++) {
      cmd.append("#IndexCards", UIPaths.ADMIN_ANNOUNCEMENT_EVENT_TOGGLE);
      String idx = "#IndexCards[" + (t + 1) + "]";
      cmd.set(idx + " #ToggleLabel.Text", toggles[t][0]);
      cmd.set(idx + " #ToggleBtn.Text", toggles[t][1]);

      events.addEventBinding(
          CustomUIEventBindingType.Activating, idx + " #ToggleBtn",
          EventData.of("Button", toggles[t][2]), false
      );
    }
    ...
    int baseIdx = toggles.length + 2; // 1 for header, 3 for toggles, 1 for divider

    for (int i = 0; i < evts.size(); i++) {
      AnnouncementEvent evt = evts.get(i);
      cmd.append("#IndexCards", UIPaths.ADMIN_ANNOUNCEMENT_EVENT_ENTRY);
      String idx = "#IndexCards[" + (baseIdx + i) + "]";

      cmd.set(idx + " #EventType.Text", evt.eventType().replace("_", " "));
      String typeBadge = evt.type() == AnnouncementType.NOTIFICATION ? "NOTIF" : "CHAT";
      cmd.set(idx + " #TypeBadge.Text", typeBadge);
      cmd.set(idx + " #EvtMsg.Text", UIHelper.truncate(evt.message(), 40));

      String enabledLabel = evt.enabled() ? "ON" : "OFF";
      cmd.set(idx + " #EvtToggle.Text", enabledLabel);

      events.addEventBinding(
          CustomUIEventBindingType.Activating, idx + " #EvtToggle",
  ...
  public void handleDataEvent(@NotNull Ref<EntityStore> ref, @NotNull Store<EntityStore> store,
    ...
    switch (data.button) {
      ...
      case "TypeChanged" -> handleTypeChanged(data.filterValue);
      ...
      case "PrevPage" -> { currentPage = Math.max(0, currentPage - 1); rebuildContent(); }
      case "NextPage" -> { currentPage++; rebuildContent(); }
      default -> sendUpdate();
    }
  }
  ...
  private void handleTypeChanged(@Nullable String typeName) {
    if (typeName != null) {
      try {
        AnnouncementType selected = AnnouncementType.valueOf(typeName);
        editTypeIndex = selected.ordinal();
      } catch (IllegalArgumentException ignored) {}
    }
    // No rebuild needed — dropdown updates itself
  }
